...
- **DanbooruGalleryNode**: 
  - 新增"智能推荐"功能：基于收藏的标签和图片，分析用户偏好（标签频次加权），推荐高质量相关图片。支持 Danbooru 认证用户的深度偏好分析。
  - 新增“收藏标签”功能：现在可以对画师（artist）、作品（copyright）和人物（character）标签进行收藏，并在左侧专用的“收藏标签”侧边栏中分类查看和快速检索。
  - 修复了因为网络或 Danbooru 账号未登录导致本地收藏数据在启动时无法正常加载的同步问题。
  - 优化画师标签输出格式，当选择输出画师时，自动在画师名前添加 `@` 符号，并将下划线 `_` 替换为空格。
